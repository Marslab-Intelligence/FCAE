...
import { pushLeadToCrm, toCrmIndustry, isCrmConfigured } from '@/lib/crm';
...
export async function submitLeadAction(input: LeadInput): Promise<LeadResult> {
  ...
  try {
    ...
    // Push into the sales engine so a salesperson can pick the lead up. This is
    // deliberately best-effort: the client has already given us their details
    // and must not see an error because the CRM is down. The outcome is
    // recorded on the row so a failed hand-off can be retried later.
    if (isCrmConfigured()) {
      const d = parsed.data;
      ...
      await db
        ...
        .set(
          result.ok
            ? { crmLeadId: result.crmLeadId || 'synced', crmSyncedAt: new Date(), crmSyncError: null }
            : { crmSyncError: result.error },
        )
        .where(eq(leads.id, row.id))
        .catch((e) => console.error('[Leads] could not record CRM sync state:', e));

      if (!result.ok) console.error('[Leads] CRM hand-off failed:', result.error);
    }
