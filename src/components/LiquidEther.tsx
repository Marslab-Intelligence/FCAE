...
/* eslint-disable @typescript-eslint/no-explicit-any -- ported WebGL fluid-sim engine; shader-uniform config objects are dynamically shaped, so precise typing would require rewriting the underlying simulation classes */
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class AutoDriver {
      ...
      constructor(mouse: MouseClass, manager: { lastUserInteraction: number }, opts: { enabled: boolean; speed: number; resumeDelay?: number; rampDuration?: number }) {
        this.mouse = mouse;
...
`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      constructor(simProps: any) {
        ...
        this.initSim(simProps);
      }

      initSim(simProps: any) {
        super.init();
    ...
    class Simulation {
      ...
      calcSize() {
        const width = Math.max(1, Math.round(Number(this.options.resolution) * Common.width));
        const height = Math.max(1, Math.round(Number(this.options.resolution) * Common.height));
      ...
      update() {
        ...
        this.advection?.update({
          dt: Number(this.options.dt),
          isBounce: Boolean(this.options.isBounce),
          BFECC: Boolean(this.options.BFECC)
        });
        this.externalForce?.update({
          cursor_size: Number(this.options.cursor_size),
          mouse_force: Number(this.options.mouse_force),
          cellScale: this.cellScale
        });
        let vel: THREE.WebGLRenderTarget | null | undefined = this.fbos.vel_1;
        if (this.options.isViscous && this.viscous) {
          vel = this.viscous.update({
            viscous: Number(this.options.viscous),
            iterations: Number(this.options.iterations_viscous),
            dt: Number(this.options.dt)
          });
        }
        if (vel) this.divergence?.update({ vel });
        const pressure = this.poisson?.update({
          iterations: Number(this.options.iterations_poisson)
        });
    ...
    class WebGLManager {
      ...
      constructor(props: Record<string, unknown>) {
        ...
        Mouse.autoIntensity = Number(props.autoIntensity);
        Mouse.takeoverDuration = Number(props.takeoverDuration);
        ...
        this.autoDriver = new AutoDriver(Mouse, this, {
          enabled: Boolean(props.autoDemo),
          speed: Number(props.autoSpeed),
          resumeDelay: Number(props.autoResumeDelay),
          rampDuration: Number(props.autoRampDuration)
        });
        this.init();
    ...
    try {
      ...
      console.warn('LiquidEther: WebGL is unavailable in this browser; skipping the animated background.', err);
      return;
    }
